...
当执行 `new User("Tom")` 时，新对象的原型会指向 `User.prototype`，所以实例可以访问 `say` 方法。这样多个实例不用每个都保存一份 `say`，而是共享原型上的方法。

```ts
const user = new (User as any)("Tom");
...
```ts
const promiseAll = <T>(tasks: Promise<T>[]) =>
