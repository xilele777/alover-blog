...
┌───────────────────────────────────────────┐
│  AI 前沿周报                                │ ← 页头
│  每周一期 · 共 N 期 · 累计 12.3万字   订阅 ⇢ │ ← 副标题 + 订阅按钮
├───────────────────────────────────────────┤
│  │ 周报第 001 期   08.13 ~ 08.19    1.1万字 │ ← 最新一期：节点主题色
│  │ 周报第 002 期   08.06 ~ 08.12    9,874字 │
│  │ 周报第 003 期   07.29 ~ 08.05    8,231字 │ ← 统一扁平行，无卡片
│  │ …                                        │ ← 容器挂轴线，两端渐隐
└───────────────────────────────────────────┘
```

- 复用 `archive.vue` 的视觉语言（`gradient-card`、`dim-hover`），每期一行，`white-space: nowrap` 保证不换行，行距比归档页更宽。
- 每行内容：**周报第 NNN 期**（期号解析失败则退化为完整标题）+ 日期区间（年份自动剥离，如 `08.13 ~ 08.19`）+ 字数（`formatNumber` 的 `x万` 形式，右对齐，`tabular-nums`）。
- 期号解析失败时不显示期号徽记；日期区间缺失时按 `date` 前推 7 天兜底。
- 最新一期节点用主题色高亮，次要信息（区间、字数）默认 `opacity 0.4`，hover/focus 时补齐。
- 时间轴轴线挂在列表容器（`weekly-list::before`）而非单项上——项间距不会切断轴线，两端渐隐代替首尾裁剪，只有一期时也能看出是时间轴。节点用与背景同色的描边把轴线切断，形成缝隙。
- 整行可点击，跳转 `article.path`；`description` 作 hover 的 `title` 提示，不在行内展示。
- 空状态：无任何周报时展示 `ZError`，不渲染空时间轴。
...
`app/pages/index.vue` 在 `listRaw` 之后、`usePublishedArticles` 之前插入过滤，剔除 `path` 以 `/weekly/` 前缀的条目（`useArticleIndexOptions` 的 `select` 不含 `stem`，只能用 `path` 过滤）。`archive.vue` 与 `tags.vue` **保持包含**周报——归档与标签本就应覆盖全站内容。
...
2. `/weekly` 以统一扁平行 + 时间轴展示，期号与日期范围解析正确，最新一期节点高亮。
3. 点击任一期进入文章页，正文应用 `.md-weekly` 排版，TOC 覆盖 `h2`/`h3`（`toc: { depth: 4, searchDepth: 4 }`），深浅色模式均正常。
